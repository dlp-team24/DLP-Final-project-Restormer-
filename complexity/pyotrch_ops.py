...
from compressai.layers.layers import (
    Attention, FeedForward,      # Restormer Block
    RSTB, BasicLayer, SwinTransformerBlock, WindowAttention, # RSTB
)
...
def WA_counter_hook(module, input, output):
    # input: [bs, C, H, W]
    flops = module.flops() * input[0].shape[0] # flops(C) * flops(bs)
    module.__flops__ += int(flops)

def CNAT_counter_hook(module, input, output):
    flops = 0

    B, Hp, Wp, C = input[0].shape
    H, W = int(Hp), int(Wp)
    ...
    if window_size is None:
        dilation = max(min(H, W) // module.kernel_size, 1)
        window_size = dilation * module.kernel_size

    if H < window_size or W < window_size:
        pad_l = pad_t = 0
        pad_r = max(0, window_size - W)
        pad_b = max(0, window_size - H)
        H, W = pad_t + H + pad_b, pad_l + W + pad_r
    
    # Generate qkv for x
    flops += B * H * W * C**2 * 3
    # Generate qkv for cond
    flops += B * H * W * C**2 * 3
    # natten2dqkrpb
    flops += (B * module.num_heads * H * W * module.head_dim * module.kernel_size**2 + 
              B * module.num_heads * H * W * module.kernel_size**2) * 4
    # natten2dav
    flops += (B * module.num_heads * H * W * module.head_dim * module.kernel_size**2) * 4
    # proj
    flops += (B * Hp * Wp * C**2) * 2
    module.__flops__ += int(flops)
...
MODULES_MAPPING = {
    ...
    # Swin Transformer
    RSTB: WA_counter_hook,
    BasicLayer: WA_counter_hook,
    SwinTransformerBlock: WA_counter_hook,
    WindowAttention: WA_counter_hook,

    # Restormer Block (MDTA) Attention
    Attention: WA_counter_hook,
    FeedForward: WA_counter_hook,
}
