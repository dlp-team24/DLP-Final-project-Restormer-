...
# TIC with Restormer Block ver.2 (g_a5, g_a7, g_s0, g_s2)
class TICwRSB_v2(nn.Module):
    """
    ...
    def __init__(self, N=128, M=192, prompt_config=None, input_resolution=(256,256), in_channel=3):
        ...
        self.g_s0 = nn.Sequential(*[RestormerBlock(dim=M,
                                                   num_heads=num_heads[2],
                                                   ffn_expansion_factor=2.66,
                                                   bias=False,
                                                   LayerNorm_type='WithBias') for i in range(depths[2])])
        self.g_s1 = deconv(M, N, kernel_size=3, stride=2)
        self.g_s2 = nn.Sequential(*[RestormerBlock(dim=N,
                                                   num_heads=num_heads[3],
                                                   ffn_expansion_factor=2.66,
                                                   bias=False,
                                                   LayerNorm_type='WithBias') for i in range(depths[3])])
    ...
    def g_a(self, x, x_size=None):
        ...
        attns.append(attn)
        x = self.g_a4(x)
        x = self.g_a5(x)     # attn, (x_size[0]//8, x_size[1]//8))
        # attns.append(attn)
        x = self.g_a6(x)
        x = self.g_a7(x)     # attn, (x_size[0]//16, x_size[1]//16))
        # attns.append(attn)
        return x, attns
    ...
    def g_s(self, x, x_size=None):
        ...
        x = self.g_s0(x)    # attn, (x_size[0]//16, x_size[1]//16))
        # attns.append(attn)
        x = self.g_s1(x)
        x = self.g_s2(x)    # attn, (x_size[0]//8, x_size[1]//8))
        # attns.append(attn)
        x = self.g_s3(x)
...
# TIC with Restormer Block ver.1 (g_a7, g_s0)
class TICwRSB_v1(nn.Module):
    """
    Modified from TIC (Lu et al., "Transformer-based Image Compression," DCC2022.)
    """
    def __init__(self, N=128, M=192, prompt_config=None, input_resolution=(256,256), in_channel=3):
...
# TIC with Restormer Block using MLP rather GDFN ver.1 (g_a7, g_s0)
class TICwRSBwMLP_v1(nn.Module):
    """
    Modified from TIC (Lu et al., "Transformer-based Image Compression," DCC2022.)
    """
    def __init__(self, N=128, M=192, prompt_config=None, input_resolution=(256,256), in_channel=3):
        ...
        self.g_a6 = conv(N, M, kernel_size=3, stride=2)
        self.g_a7 = nn.Sequential(*[RestormerBlock2(dim=M,
                                                   num_heads=num_heads[3],
                                                   bias=False,
        ...
        self.g_s0 = nn.Sequential(*[RestormerBlock2(dim=M,
                                                   num_heads=num_heads[2],
                                                   bias=False,
                                                   LayerNorm_type='WithBias',
                                                   mlp_ratio=mlp_ratio,
                                                   drop=drop_rate,
                                                   act_layer=nn.GELU) for i in range(depths[2])])
        self.g_s1 = deconv(M, N, kernel_size=3, stride=2)
        self.g_s2 = RSTB(dim=N,
                        input_resolution=(input_resolution[0]//8, input_resolution[1]//8),
                        depth=depths[3],
    ...
    def decompress(self, strings, shape):
        ...
        x_hat, attns_s = self.g_s(y_hat).clamp_(0, 1)
        return {"x_hat": x_hat}
# TIC with one Restormer Block with only MDTA ver.1 (g_a7, g_s0)
class TICwRSBwoMLP_v1(nn.Module):
    """
    Modified from TIC (Lu et al., "Transformer-based Image Compression," DCC2022.)
    """
    def __init__(self, N=128, M=192, prompt_config=None, input_resolution=(256,256), in_channel=3):
        ...
        self.g_a6 = conv(N, M, kernel_size=3, stride=2)
        self.g_a7 = nn.Sequential(*[RestormerBlock3(dim=M,
                                                   num_heads=num_heads[3],
                                                   bias=False,
                                                   LayerNorm_type='WithBias',
        ...
        self.g_s0 = nn.Sequential(*[RestormerBlock3(dim=M,
                                                   num_heads=num_heads[2],
                                                   bias=False,
                                                   LayerNorm_type='WithBias',
    ...
    def decompress(self, strings, shape):
        ...
        x_hat, attns_s = self.g_s(y_hat).clamp_(0, 1)
        return {"x_hat": x_hat}
# TIC with last RSTB which is modified the last layer to MLP version Restormer ver.1 (g_a7, g_s0)
class TICwRSTB_RSBwMLP_v1(nn.Module):
    """
    Modified from TIC (Lu et al., "Transformer-based Image Compression," DCC2022.)
    """
    def __init__(self, N=128, M=192, prompt_config=None, input_resolution=(256,256), in_channel=3):
    ...
    def g_a(self, x, x_size=None):
        ...
        if x_size is None:
            x_size = x.shape[2:4]
        x = self.g_a0(x)
        #print("after ga0:",x.shape)
        x, attn = self.g_a1(x, (x_size[0]//2, x_size[1]//2))
        #print("after ga1:",x.shape)
        attns.append(attn)
        x = self.g_a2(x)
        #print("after ga2:",x.shape)
        x, attn = self.g_a3(x, (x_size[0]//4, x_size[1]//4))
        #print("after ga3:",x.shape)
        attns.append(attn)
        x = self.g_a4(x)
        #print("after ga4:",x.shape)
        x, attn = self.g_a5(x, (x_size[0]//8, x_size[1]//8))
        #print("after ga5:",x.shape)
        attns.append(attn)
        x = self.g_a6(x)
        #print("after ga6:",x.shape)
        x, attn = self.g_a7(x, (x_size[0]//16, x_size[1]//16))
        #print("after ga7:",x.shape)
        attns.append(attn)
        return x, attns
